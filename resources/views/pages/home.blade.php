...
<section class="section" x-data="countryTabs()">
    <div class="wrap">
        ...
        @php
        $panels = [
            'china'     => ['name'=>'China',               'label'=>'BEIJING · 北京',       'phA'=>'#a51717','phB'=>'#3d0808','image'=>'china.jpg',      'blurb'=>'The largest higher-education system in the world. World-class research, generous scholarships and the fastest-growing alumni network in Asia.',       'stats'=>[['280+','Partner universities'],['1,200+','Programmes'],['¥0','Tuition (full-ride)'],['48 hrs','Avg. response']], 'unis'=>[['Zhejiang Univ. of Science & Technology','Hangzhou · Zhejiang · ZUST',route('china.zust')],['Shandong University of Technology','Zibo · Shandong · SDUT',route('china.sdut')],['Jiangxi Univ. of Finance & Economics','Nanchang · Jiangxi · JUFE',route('china.jufe')],['Hainan Medical University','Haikou · Hainan · HMU',route('china.hmu')]], 'href'=>route('china'),    'zh'=>'中国'],
            'malaysia'  => ['name'=>'Malaysia',            'label'=>'KUALA LUMPUR · KL',    'phA'=>'#0a1f5e','phB'=>'#061240','image'=>'slide-klcc.jpg', 'blurb'=>'English-medium instruction, multicultural campuses and tuition at a fraction of UK/AU costs. A natural bridge for ASEAN and South Asian students.','stats'=>[['45+','Partner universities'],['380+','Programmes'],['RM 18k','From / year'],['72 hrs','Avg. response']],  'unis'=>[['Universiti Malaya','KL · QS #60'],['Universiti Putra Malaysia','Selangor · QS #148'],['Taylor\'s University','Subang · QS #251'],['Sunway University','Bandar Sunway · QS #586'],['Monash University Malaysia','Bandar Sunway · QS Asia #44']], 'href'=>route('malaysia'), 'zh'=>'马来西亚'],
            'indonesia' => ['name'=>'Indonesia',           'label'=>'JAKARTA · ID',         'phA'=>'#c98a1d','phB'=>'#5e3f10','image'=>'jakarta.jpg',   'blurb'=>'Newly opened destination with strong programmes in business, hospitality and Southeast Asian studies. Direct cohorts launching from 2026.',          'stats'=>[['32+','Partner universities'],['220+','Programmes'],['IDR 60M','From / year'],['96 hrs','Avg. response']],  'unis'=>[['Universitas Indonesia','Depok · QS #237'],['Institut Teknologi Bandung','Bandung · QS #281'],['Universitas Gadjah Mada','Yogyakarta · QS #239'],['Universitas Airlangga','Surabaya · QS #345'],['Bina Nusantara University','Jakarta · QS #701']], 'href'=>'#',               'zh'=>'印度尼西亚'],
            'future'    => ['name'=>'Future Destinations', 'label'=>'EXPANDING · ASIA',     'phA'=>'#3a3f56','phB'=>'#15182a','image'=>'singapore.jpg',   'blurb'=>'Coming soon — Singapore, Hong Kong, Thailand and Vietnam. Join the waitlist and be first to access pilot cohorts in 2027.',                        'stats'=>[['4','New destinations'],['2027','Pilot launch'],['Free','Waitlist'],['—','Coming soon']],                   'unis'=>[['Singapore','Pilot · 2027'],['Hong Kong SAR','Pilot · 2027'],['Thailand','Pilot · 2027'],['Vietnam','Pilot · 2028'],['Korea (under review)','Pilot · 2028']], 'href'=>'#',               'zh'=>'未来'],
        ];
        @endphp
        ...
        <div style="display:grid; grid-template-columns:1fr 280px 280px; gap:0; border:1px solid var(--rule-soft); border-top:none; background:var(--paper);">
            ...
            {{-- Col 2: Image panel --}}
            <div style="position:relative; min-height:300px; overflow:hidden; background:linear-gradient(160deg, {{ $p['phA'] }}, {{ $p['phB'] }});">
                @if(!empty($p['image']))
                <img src="{{ asset('assets/'.$p['image']) }}" alt="{{ $p['name'] }}"
                     style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;">
                <div style="position:absolute; inset:0; background:linear-gradient(160deg, {{ $p['phA'] }}aa, {{ $p['phB'] }}88);"></div>
                @else
                ...
                <div style="position:absolute; bottom:0; left:0; right:0; display:flex; justify-content:space-between; align-items:flex-end; padding:14px 16px;">
                    <span style="font-family:'JetBrains Mono',monospace; font-size:9.5px; letter-spacing:0.15em; color:rgba(255,255,255,0.6); text-transform:uppercase; text-shadow:0 1px 3px rgba(0,0,0,0.5);">{{ $p['label'] }}</span>
                    <span class="zh" style="font-size:14px; color:rgba(255,255,255,0.5); letter-spacing:0.06em; text-shadow:0 1px 3px rgba(0,0,0,0.5);">{{ $p['zh'] }}</span>
                </div>
            </div>
            ...
            <div style="border-left:1px solid var(--rule-soft); padding:24px;">
                ...
                @foreach($p['unis'] as $i => $uni)
                @php [$name,$city,$link] = array_pad($uni, 3, null); @endphp
                @if($link)
                <a href="{{ $link }}" style="display:flex; gap:12px; align-items:baseline; padding:10px 0; border-bottom:1px solid var(--rule-soft); text-decoration:none; color:inherit;">
                @else
                ...
                @endif
                    ...
                    <div style="flex:1; min-width:0;">
                        <div style="font-family:'Instrument Serif',serif; font-size:16px; color:{{ $link ? 'var(--accent)' : 'var(--ink)' }}; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ $name }}</div>
